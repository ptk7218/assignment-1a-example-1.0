...
public class HandleFishingHook : MonoBehaviour {
    ...
    private bool enableFishinRodControll=true;
    [SerializeField]
    ...
    void Update() {
        if (Input.GetKey(KeyCode.Space)&&enableFishinRodControll||Input.touchCount > 0&&enableFishinRodControll) {
        ...
        if(endPoint.position.y >= initialEndPointY){
            enableFishinRodControll=true;
            hookSprite.sprite=hookWithBait;
        }
        DrawFishingLine();
    }
    ...
    public void setEnableFishingRodControll(bool b){
        enableFishinRodControll=b;
    }
    public bool getFishingRodControll(){
        return enableFishinRodControll;
    }
}
